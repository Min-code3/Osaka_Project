...
# ---------------------------------------------------------
# [관리자 기능] 사이드바 강제 노출 (Debugging)
# ---------------------------------------------------------

# 👇 이 코드는 들여쓰기 없이 '맨 왼쪽' 벽에 붙여야 합니다!
with st.sidebar:
    st.divider()
    st.error("🚨 관리자 모드 (사이드바)") # 빨간색 박스로 강조
    
    # 1. 비밀번호 입력 (무조건 보이게 함)
    input_pw = st.text_input("🔑 관리자 암호", type="password")
